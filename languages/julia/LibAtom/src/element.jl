#BARELY BEGUN. We could mimick the design of the python api in Julia, TBD
...
struct Element
    ...
    streams::Set{String}
    _rclient
    _command_loop_shutdown
    _rpipeline_pool
    reserved_commands
    response_last_id = _pipe.execute()[-1].decode()
    response_last_id_lock = threading.Lock()
    # Keep track of command_last_id to know last time the element's command stream was read from
    command_last_id = _pipe.execute()[-1].decode()
end

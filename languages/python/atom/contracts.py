...
from lazycontract.contract import LazyContractValidationError, LazyContractDeserializationError
...
class RawContract(LazyContract):
    def __init__(self,  *args, **kwargs):
        if len(args) + len(kwargs) != 1 or (len(kwargs) == 1 and "data" not in kwargs):
            raise LazyContractValidationError("Raw contracts must store a single data value")
        ...
        if "data" not in self._properties or not self._properties["data"].required or len(self._properties) > 1:
            raise LazyContractValidationError("Raw contracts must specify a required data field name. No other fields are allowed.")
...
class EmptyContract(LazyContract):
    def __init__(self, *args, **kwargs):
        ...
        if (len(args) == 1 and args[0] != "" and args[0] != b""):
            is_empty = False
        if (len(kwargs) == 1 and ("data" not in kwargs or (kwargs["data"] != "" and kwargs["data"] != b""))):
            is_empty = False

        if not is_empty or len(self._properties) > 0:
            raise LazyContractValidationError("Empty contract should contain no data and no field definitions")

    def to_dict(self):
        raise TypeError("Cannot convert empty contract to dict, use the to_data() function instead")
